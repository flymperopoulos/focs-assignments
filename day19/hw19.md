...
1. ∃x. ∀y. loves(x, y) - False, nobody loves everyone.  
2. ∃y. ∀x. loves(x, y) - True, everybody loves c.  
3. ∀x. ∃y. loves(x, y) - True, everybody loves someone.  
4. ∀y. ∃x. loves(x, y) - True, everybody is being loved by someone.  
5. ∃x. ∃y. loves(x, y) - True, someone loves someone.  
6. ∃x. ∀y. ¬loves(x, y) - False, someone loves someone - nobody doesn't love anyone.  
7. ∃x. ¬∀y. loves(x, y) - True, there exists someone that doesn't love everyone (a doesn't love d)  
...
if y = s(0), then we have sum(s(0), 0, s(0)). This implies that sum(y, s(0), s(y)) = sum(s(0), s(0), s(s(0)))  

2.  ∀x. sum( 0, x, x ) [hint:  induction]  

For i=1, we have that sum(0,1,1) is true. By inductive hypothesis we assume that for all k sum(0,k,k) is true. We have to prove that this is true for k+1. Which esentially is sum(0, s(k), s(k)), and sum(0,k,k) --> sum(0, s(k), s(k)).  
...
What? there is no recursion graph for that function.  

b. Is `max_subarray` an example of divide and conquer? Why or why not?  

Based on previous answer, it is not recursive so no.  

c. Is `max_subarray` an example of dynamic programming? Why or why not?  

No, as no memoization is present.  
...
No recursion so no need for it.  
...
binary_search_array_helper(3, [1, 3, 4, 6, 7, 8, 10, 13, 14], 0, 9) --> [3 < 7]
binary_search_array_helper(3, [1, 3, 4, 6, 7, 8, 10, 13, 14], 0, 4) --> [3 < 7]
binary_search_array_helper(3, [1, 3, 4, 6, 7, 8, 10, 13, 14], 0, 2) --> [3 > 2]
```

b. Is `binary_search_array` an example of divide and conquer? Why or why not?  

Yes, since it's recursive.  

c. Is `binary_search_array` an example of dynamic programming? Why or why not?  

No. It does not remember the previous problem - it resolves it.  
...
Isn't it the same thing as (a) but with a different function name?  
...
I am not getting this. I would assume that the memoized version would be able to "look up" the value it computed previously and use it instead of recalling this helper again?  
